...
const getList = async(start, count) =>{
    const sql = 
        "SELECT B.id AS id, B.title AS title, B.created_at AS createdAt, B.last_updated AS lastUpdated, "+
        "A.display_name AS displyName FROM users AS A INNER JOIN articles AS B ON A.id = B.author "+
...
const getById = async(id) =>{
    const sql= 
        "SELECT B.id AS id, B.title AS title, B.content AS content, B.created_at AS createdAT, B.last_updated AS lastUpadted, "+
        "B.author AS author, A.display_name AS displyName FROM users AS A INNER JOIN articles AS B ON A.id = B.author "+
...
const create = async(title, content, author) => {
    ...
    var res = await runQuery(sql, [title, content, author]);
    return res.insertId;
}

const update = async(id, title, content) => {
    const sql=
        "UPDATE articles SET title = ?, content=? WHERE articles WHERE id = ?";
